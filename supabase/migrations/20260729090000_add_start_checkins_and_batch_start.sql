create table if not exists public.start_checkins (
  ...
  constraint start_checkins_race_participant_key unique (race_id, participant_id),
  constraint start_checkins_device_id_check
    check (char_length(btrim(device_id)) between 1 and 100),
  constraint start_checkins_status_check
    check (status in ('ready', 'started')),
  constraint start_checkins_started_at_check
    check (
...
create index if not exists start_checkins_ready_queue_idx
  on public.start_checkins (race_id, confirmed_at desc, participant_id)
  where status = 'ready';

alter table public.start_checkins enable row level security;

revoke all on public.start_checkins from anon, authenticated;
grant select, insert, update, delete on public.start_checkins to anon;

create policy start_checkins_server_select
  on public.start_checkins for select to anon
  using ((select private.timing_api_authorized()));

create policy start_checkins_server_insert
  on public.start_checkins for insert to anon
  with check ((select private.timing_api_authorized()));

create policy start_checkins_server_update
  on public.start_checkins for update to anon
  using ((select private.timing_api_authorized()))
  with check ((select private.timing_api_authorized()));

create policy start_checkins_server_delete
  on public.start_checkins for delete to anon
  using ((select private.timing_api_authorized()));
...
begin
  ...
  select jsonb_agg(
    ...
    order by participant.athlete_name, participant.id
  )
  into v_started
  from public.participants participant
  where participant.race_id = p_race_id
